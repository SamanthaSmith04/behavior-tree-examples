...
// ---------- Path Segment ----------
template<>
struct convert<plg_msgs::msg::PathSegment>
{
  static Node encode(const plg_msgs::msg::PathSegment& rhs)
  {
    Node node;
    node["lead_in"] = YAML::convert<geometry_msgs::msg::PoseArray>::encode(rhs.lead_in);
    node["lead_out"] = YAML::convert<geometry_msgs::msg::PoseArray>::encode(rhs.lead_out);
    node["segment"] = YAML::convert<geometry_msgs::msg::PoseArray>::encode(rhs.segment);

    node["velocity"] = rhs.velocity;
    node["motion_profile"] = static_cast<int>(rhs.motion_profile);
    node["tool_frame"] = rhs.tool_frame;
    return node;
  }

  static bool decode(const Node& node, plg_msgs::msg::PathSegment& rhs)
  {
    if (!node["lead_in"] || !node["lead_out"] || !node["segment"] ||
        !node["velocity"] || !node["motion_profile"] || !node["tool_frame"]) {
      return false;
    }
    rhs.lead_in = node["lead_in"].as<geometry_msgs::msg::PoseArray>();
    rhs.lead_out = node["lead_out"].as<geometry_msgs::msg::PoseArray>();
    rhs.segment = node["segment"].as<geometry_msgs::msg::PoseArray>();

    rhs.velocity = node["velocity"].as<double>();
    rhs.motion_profile = static_cast<MotionProfile>(node["motion_profile"].as<int>());
    rhs.tool_frame = node["tool_frame"].as<std::string>();
    return true;
  }
};
...
// ---------- Tool Path ----------
template<>
struct convert<plg_msgs::msg::ToolPath> 
{
  static Node encode(const plg_msgs::msg::ToolPath& rhs)
  {
    Node node;
    for (const auto& path_segment : rhs.path) {
      node["path"].push_back(YAML::convert<plg_msgs::msg::PathSegment>::encode(path_segment));
    }
    return node;
  }
  static bool decode(const Node& node, plg_msgs::msg::ToolPath& rhs)
  {
    ...
    for (const auto& n : node["path"]) {
      rhs.path.push_back(n.as<plg_msgs::msg::PathSegment>());
    }
    return true;
  }
};
